...
type Config struct {
	...
	Transport struct {
		HTTP struct {
			...
			Router struct {
				EnabledSyncRouter bool `mapstructure:"ENABLED_SYNC_ROUTER"`
			}
		}
	}
	...
	Internal struct {
		Domain struct {
			Catalog struct {
				CatalogsDefaultSize         int `mapstructure:"CATALOGS_DEFAULT_SIZE"`
				CatalogModelListDefaultSize int `mapstructure:"CATALOG_MODEL_LIST_DEFAULT_SIZE"`
			}
			Product struct {
				ProductDefaultSize             int  `mapstructure:"PRODUCT_DEFAULT_SIZE"`
				ProductSearchIndexBatchLimit   int  `mapstructure:"PRODUCT_SEARCH_INDEX_BATCH_LIMIT"`
				EnableIndexingAllProductSearch bool `mapstructure:"ENABLE_INDEXING_ALL_PRODUCT_SEARCH"`
			}
		}
	}
}
